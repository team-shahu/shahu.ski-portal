<script setup lang="ts">
    ...
    const search_text: string = route.query?.q as string;
    let categories: any = ref([]);
    let search_input = ref(search_text);
...
<template>
    ...
    <div class="flex flex-col items-center justify-center pt-24 mx-auto rounded-lg lg:px-10 max-w-7xl">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">"{{ search_text }}"の検索結果</h1>
        <div class="flex flex-col items-center justify-center pt-24 mx-auto rounded-lg lg:px-10 max-w-7xl">
            <PostList :search_text="search_text" />
        </div>
    </div>
</template>